    ...
    setTimeout(() => {
      ...
      const newUrl =
        ...
        window.location.search
          .replace(/[?&]openModal=addAppointment/, "")
          .replace(/^&/, "?");
...
// Function to move modals to body to assume better z-index stacking
function fixModalPlacement() {
  const modals = [
    "appointmentModal",
    "addAppointmentModal",
    "cancelModal",
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("input", debounce(searchPatients, 300));
  ...
  document.getElementById("newPatientBtn").addEventListener("click", () => {
    ...
    setTimeout(() => {
      const addPatientModalElement = document.getElementById("addPatientModal");
      const addPatientModal = new bootstrap.Modal(addPatientModalElement);
      // z-index is handled by main.js
...
function renderCalendar(data) {
  ...
  if (isFriday || data.is_friday) {
    const dayName = currentDateObj.toLocaleDateString("en-US", {
      weekday: "long",
    });
    ...
    timeSlots.forEach((time) => {
      ...
      if (appointment) {
        ...
        html += `<div class="available-slot" onclick="quickAddAppointment('${time}')" 
                              title="Click to schedule appointment at ${formatTime(
                                time
                              )}">
                            <i class="bi bi-plus-circle me-2"></i>Available - ${formatTime(
                              time
                            )}
                         </div>`;
      } else {
        ...
        if (
          unavailableSlot &&
          unavailableSlot.reason === "Outside working hours"
        ) {
          ...
          const debugInfo =
            unavailableSlot && unavailableSlot.debug_info
              ? unavailableSlot.debug_info
              : `No slot data - Time: ${time}`;
          const reason =
            unavailableSlot && unavailableSlot.reason
...
// Visual identity per clinic (icon + color). Defers to ClinicsLoader so the mapping
// stays in one place (layouts/clinics-loader.js); local fallback for safety.
...
function renderAppointmentSlot(appointment) {
        ...
        <div class="appointment-tooltip" style="font-family: 'Cairo', sans-serif; !important;">
            ...
            <div class="tooltip-body">
                ...
                <div class="tooltip-row">
                    ...
                    <span class="tooltip-value">${
                      calculateAge(appointment.dob) || "N/A"
                    }</span>
                </div>
                <div class="tooltip-row">
                ...
                <div class="tooltip-row">
                    ...
                    <span class="tooltip-value">${formatTime(
  ...
  return `
    ...
    isCompleted ? "completed" : ""
                ...
                <div class="appointment-actions">
                    <button class="btn btn-sm btn-outline-warning view-medical-history-btn"
                            onclick="event.stopPropagation(); showMedicalHistoryPopover(${
    ...
    appointment.start_time
            ...
            <div class="appointment-notes" onclick="navigateToAppointment(${
              appointment.id
            })">
                ${
                  appointment.notes
...
function deleteAppointment(appointmentId, patientName, appointmentTime) {
  ...
  document.getElementById("deleteAppointmentPatientName").textContent =
    patientName;
  document.getElementById("deleteAppointmentTime").textContent =
    appointmentTime;

  // Show modal
  const deleteModalElement = document.getElementById("deleteAppointmentModal");
...
function showActionButtons(status) {
  ...
  document
    .querySelectorAll(
      "#appointmentModal .modal-footer button:not(.btn-secondary)"
    )
    .forEach((btn) => {
  ...
  switch (status) {
    ...
    case "CheckedIn":
      ...
      break;
    case "InProgress":
      document.getElementById("completeVisitBtn").style.display =
        "inline-block";
      break;
  }
}
...
function openAddAppointmentModal(
  ...
  const visitTypeFromConfig =
    window.CALENDAR_CONFIG?.visitType ||
    window.CALENDAR_CONFIG?.preselectedVisitType;
  ...
  setTimeout(() => {
    ...
    const visitTypeButton = visitTypeField?.querySelector(
    ...
    if (
      ...
      visitTypeSelect.querySelector(`option[value="${preselectedVisitType}"]`)
    ) {
      // Use preselected visit_type from another page
      visitTypeSelect.value = preselectedVisitType;
      if (visitTypeButton && visitTypeMenu) {
        const selectedOption = visitTypeMenu.querySelector(
          `li[data-option="${preselectedVisitType}"]`
        );
        if (selectedOption) {
          visitTypeMenu
            .querySelectorAll("li")
          ...
          const optionText =
            selectedOption.querySelector("h3")?.textContent ||
            preselectedVisitType;
          if (optionIcon) {
            visitTypeButton.innerHTML =
              optionIcon.outerHTML + " <h3>" + optionText + "</h3>";
          } else {
            ...
            <div class="selected-patient-info alert alert-info">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        ...
                        <small>Phone: ${preselectedPatient.phone} • Age: ${
      preselectedPatient.age || "N/A"
    }</small>
                    </div>
  ...
  // Show modal - z-index is handled by main.js
  ...
  modalElement.addEventListener("hidden.bs.modal", function () {
    document
      ...
      .classList.remove("preselected-field");
    document
...
function quickAddAppointment(time) {
  ...
  if (isDateInPast(selectedDate)) {
    showErrorMessage(
      "Cannot add appointment on a past date. Please select today or a future date."
    );
    return;
  }
...
function validateDateSelection(dateString) {
  if (isDateInPast(dateString)) {
    return {
      valid: false,
      message:
        "Cannot select a date before today. Current date (Egypt timezone): " +
        formatDateArabic(SERVER_DATE),
    };
  }
  return { valid: true };
}
...
function displayPatientSearchResults(patients) {
  ...
  if (patients.length === 0) {
    resultsContainer.innerHTML =
      '<div class="search-result-item text-muted">No patients found</div>';
    return;
  }
  ...
  patients.forEach((patient) => {
    html += `
            <div class="search-result-item" onclick="selectPatient(${
              patient.id
            }, '${patient.first_name} ${patient.last_name}')">
                <div class="patient-name">${patient.first_name} ${
      patient.last_name
    }</div>
                <div class="patient-details">${patient.phone} • Age: ${
      patient.age || "N/A"
    }</div>
            </div>
        `;
...
function populateTimeSlots(availableSlots, preselectedTime = null) {
  ...
  const timeButton = timeField
    ? timeField.querySelector(".custom-select-toggle")
    : null;
  ...
  if (timeMenu) {
    timeMenu.innerHTML =
      '<li data-option="" tabindex="0" role="button" class="selected"><h3>Select time slot...</h3></li>';
    ...
    newOptions.forEach((option) => {
      option.addEventListener("click", (e) => {
        ...
        const text =
          option.querySelector("h3")?.textContent || option.textContent;
    ...
    setTimeout(() => {
      ...
      if (timeSelect.value === preselectedTime) {
        ...
        if (timeMenu && timeButton) {
          const selectedLi = timeMenu.querySelector(
            `li[data-option="${preselectedTime}"]`
          );
          if (selectedLi) {
            timeMenu
              .querySelectorAll("li")
            ...
            timeButton.textContent =
              selectedLi.querySelector("h3")?.textContent ||
              formatTime(preselectedTime);
...
function handleAddAppointment(e) {
  ...
  if (appointmentData.date === today && appointmentData.patient_id) {
    ...
    fetch(
      `/api/patients/${appointmentData.patient_id}/appointments/check-active`
    )
      .then((checkResponse) => checkResponse.json())
      .then((checkData) => {
        if (checkData.has_active) {
          showErrorMessage(
            "This patient already has an appointment scheduled for today. Please complete or cancel the existing appointment first."
          );
          return;
        }
...
function proceedWithAppointmentCreation(appointmentData) {
  ...
  fetch("/api/appointments", {
    ...
    .catch((error) => {
      ...
      const errorMessage =
        error.message || "Error saving appointment: " + error.message;
      showNotification(errorMessage, "danger");
    });
}
...
function initializeAddPatientModal() {
  ...
  addPatientModal.addEventListener("show.bs.modal", function () {
    ...
    // Hide phone validation alert
    const phoneAlert = document.getElementById("phoneValidationAlert");
    if (phoneAlert) {
  ...
  addPatientForm.addEventListener("submit", function (e) {
    ...
    // Validate phone number format - must start with 0 and be exactly 11 digits
    const phoneRegex = /^0\d{10}$/;
    if (!phoneRegex.test(phone)) {
      showMessage(
        "Phone number must start with 0 and consist of exactly 11 digits (e.g., 01234567890).",
        "error"
      );
      return;
    }
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              if (patientData && (patientData.id || patientData.patient_id)) {
                ...
                showNotification(
                  "Patient added but could not auto-select. Please search for the patient manually.",
                  "warning"
                );
              }
            }, 300);
  ...
  function selectNewPatient(patientData) {
    ...
    const age =
      patientData.age || calculateAgeFromDOB(patientData.dob) || "N/A";
  ...
  function showMessage(message, type) {
    addPatientMessage.className = `alert alert-${
  ...
  addPatientForm.addEventListener("input", function (e) {
    ...
    // Hide phone validation alert when user starts typing
  ...
  if (phoneInput) {
    phoneInput.addEventListener("blur", function () {
      ...
      const phoneAlert = document.getElementById("phoneValidationAlert");
      const phoneAlertMessage = document.getElementById("phoneValidationMessage");
      
      if (!phone) {
        // Empty phone - don't show alert, let required validation handle it
        if (phoneAlert) {
          phoneAlert.classList.add("d-none");
        }
        return;
      ...
      if (!phoneRegex.test(phone)) {
        ...
        if (phoneAlert) {
          if (phone.length < 11) {
            phoneAlertMessage.textContent = "Phone number must be exactly 11 digits.";
          } else if (!phone.startsWith("0")) {
            phoneAlertMessage.textContent = "Phone number must start with 0.";
          } else if (!/^\d+$/.test(phone)) {
            phoneAlertMessage.textContent = "Phone number must contain only digits.";
          } else {
            phoneAlertMessage.textContent = "Phone number must start with 0 and consist of exactly 11 digits (e.g., 01234567890).";
          }
...
function initializeAppointmentProgressBars() {
  const progressContainers = document.querySelectorAll(
    ".appointment-progress-container"
  );
...
function updateAppointmentProgressBar(container) {
  ...
  if (!dateStr || !startTimeStr || !endTimeStr) {
    console.warn("Missing appointment data:", {
      dateStr,
      startTimeStr,
      endTimeStr,
    });
  ...
  if (nowTime < startTime) {
    ...
    if (remainingSeconds >= 3600) {
      ...
      timeValue = `${String(hours).padStart(2, "0")}:${String(minutes).padStart(
        2,
        "0"
      ...
      const seconds = remainingSeconds % 60;
      timeValue = `${String(minutes).padStart(2, "0")}:${String(
        seconds
      ).padStart(2, "0")}`;
    }
    ...
    progress =
      adjustedTotal > 0
        ? ((maxCountdownTime - adjustedTotal) / maxCountdownTime) * 100
        : 100;
    ...
    if (overdueSeconds >= secondsPerDay) {
      ...
      const minutes = Math.floor((remainingSeconds % 3600) / 60);
      const seconds = remainingSeconds % 60;
      timeValue = `${days} day${days !== 1 ? "s" : ""}, ${String(
        hours
