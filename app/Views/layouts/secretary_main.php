...
<body>
    ...
    // Per-user WhatsApp prefs — each secretary controls their own triggers.
    $__waUid = (new \App\Lib\Auth())->user()['id'] ?? 0;
    $__waSettingsStmt = $__waDb->prepare("SELECT setting_key, setting_value FROM secretary_settings WHERE user_id = ? AND setting_key IN ('whatsapp_enabled', 'whatsapp_advanced_features', 'whatsapp_mod_appointments', 'whatsapp_mod_visits', 'whatsapp_mod_report', 'whatsapp_mod_patientlog')");
